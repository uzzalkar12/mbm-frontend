...
export default defineComponent({
  ...
  setup: function () {
    ...
    const save = async () => {
      ...
      await store.dispatch(Actions.PRODUCT_REQUISITION_UPDATE, {
        product_requisition: formData,
        id: product_requisition.value.id
      }).then(() => {
    ...
    const getProducts = async () => {
      loading.value = true
      await store.dispatch(Actions.PRODUCT_REQUISITION_LIST, route.params.product_requisition_id).then(async() => {
        product_requisition.value = await JSON.parse(JSON.stringify(computed(() => store.getters.getProductRequisition).value))
